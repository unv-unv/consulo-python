...
public class PyUtil {
    ...
    @RequiredReadAction
    public static <T extends PyElement> T[] getAllChildrenOfType(PsiElement element, Class<T> aClass) {
     ...
     */
    protected static List<PyExpression> unfoldParentheses(
        ...
        for (PyExpression exp : targets) {
            if (exp instanceof PyParenthesizedExpression parenExpr) {
                unfoldParentheses(
                    new PyExpression[]{parenExpr.getContainedExpression()},
                    receiver,
                    unfoldListLiterals,
            ...
            else if (exp instanceof PyStarExpression starExpr && unfoldStarExpressions) {
                unfoldParentheses(new PyExpression[]{starExpr.getExpression()}, receiver, unfoldListLiterals, true);
            }
            else if (exp != null) {
                receiver.add(exp);
            }
    ...
    public static PyFile getContainingPyFile(PyElement element) {
        return element.getContainingFile() instanceof PyFile containingFile ? containingFile : null;
    }
     ...
     */
    public static void addListNode(
        PsiElement parent,
        PsiElement newItem,
        ASTNode beforeThis,
        boolean isFirst,
     ...
     */
    @RequiredReadAction
    public static List<PyClass> getAllSuperClasses(PyClass pyClass) {
            ...
            if (qualifier != null) {
                ...
                if (PyNames.__CLASS__.equals(attr_name)) {
                    ...
                    if (qualifierType instanceof PyClassType classType) {
                        return new PyClassTypeImpl(classType.getPyClass(), true); // always as class, never instance
                    }
                }
    ...
    public static boolean isInstanceAttribute(PyExpression target) {
        ...
        if (owner instanceof PyFunction method && method.getContainingClass() != null) {
    ...
    private static boolean isNameEqualsMain(PyExpression condition) {
        if (condition instanceof PyParenthesizedExpression parenExpr) {
            return isNameEqualsMain(parenExpr.getContainedExpression());
        }
    ...
    public static boolean inSameFile(PsiElement e1, PsiElement e2) {
        PsiFile f1 = e1.getContainingFile();
        PsiFile f2 = e2.getContainingFile();
        return !(f1 == null || f2 == null) && f1 == f2;
    }

    public static boolean onSameLine(PsiElement e1, PsiElement e2) {
        PsiDocumentManager documentManager = PsiDocumentManager.getInstance(e1.getProject());
        Document document = documentManager.getDocument(e1.getContainingFile());
    ...
    public static boolean isTopLevel(PsiElement element) {
        if (element instanceof StubBasedPsiElement stubBasedPsiElem) {
            StubElement stub = stubBasedPsiElem.getStub();
            if (stub != null) {
    ...
    public static void deletePycFiles(String pyFilePath) {
        if (pyFilePath.endsWith(PyNames.DOT_PY)) {
            ...
            File file = new File(pyFilePath);
            File pyCache = new File(file.getParentFile(), PyNames.PYCACHE);
            if (pyCache.isDirectory()) {
                String shortName = FileUtil.getNameWithoutExtension(file);
    ...
    public static boolean hasUnresolvedAncestors(PyClass cls, TypeEvalContext context) {
    ...
    public static AccessDirection getPropertyAccessDirection(PyFunction function) {
    ...
    public static void removeQualifier(PyReferenceExpression element) {
        ...
        if (qualifier instanceof PyCallExpression call && call.getCallee() instanceof PyReferenceExpression callee) {
            PyExpression calleeQualifier = callee.getQualifier();
            if (calleeQualifier != null) {
                qualifier.replace(calleeQualifier);
                return;
            }
        }
        PsiElement dot = PyPsiUtils.getNextNonWhitespaceSibling(qualifier);
        if (dot != null) {
            dot.delete();
        }
        qualifier.delete();
    ...
    public static String computeElementNameForStringSearch(PsiElement element) {
        ...
        if (element instanceof PyStringLiteralExpression stringLiteral && PyMagicLiteralTools.isMagicLiteral(element)) {
            String name = stringLiteral.getStringValue();
            if (name != null) {
                return name;
            }
        }
        if (element instanceof PyElement pyElement) {
    ...
    public static boolean isOwnScopeComprehension(PyComprehensionElement comprehension) {
    ...
    public static boolean hasCustomDecorators(PyDecoratable decoratable) {
        return PyKnownDecoratorUtil.hasNonBuiltinDecorator(decoratable, TypeEvalContext.codeInsightFallback(null));
    }

    public static boolean isDecoratedAsAbstract(PyDecoratable decoratable) {
     ...
     */
    @RequiredReadAction
    public static PsiElement resolveToTheTop(PsiElement elementToResolve) {
        PsiElement currentElement = elementToResolve;
        Set<PsiElement> checkedElements = new HashSet<>(); // To prevent PY-20553
        while (true) {
            ...
            if (resolve == null
                || checkedElements.contains(resolve)
                || resolve.equals(currentElement)
                || !inSameFile(resolve, currentElement)) {
                break;
            }
     ...
     */
    @RequiredReadAction
    public static List<PsiElement> multiResolveTopPriority(PsiElement element, PyResolveContext resolveContext) {
        if (element instanceof PyReferenceOwner refOwner) {
            PsiPolyVariantReference ref = refOwner.getReference(resolveContext);
    ...
    @RequiredReadAction
    public static List<PsiElement> multiResolveTopPriority(PsiPolyVariantReference reference) {
        return filterTopPriorityResults(reference.multiResolve(false));
    }

    public static List<PsiElement> filterTopPriorityResults(ResolveResult[] resolveResults) {
        if (resolveResults.length == 0) {
            return Collections.emptyList();
        }
        List<PsiElement> filtered = new ArrayList<>();
        int maxRate = getMaxRate(resolveResults);
        for (ResolveResult resolveResult : resolveResults) {
            int rate = resolveResult instanceof RatedResolveResult ratedResolveResult ? ratedResolveResult.getRate() : 0;
            if (rate >= maxRate) {
    ...
    private static int getMaxRate(ResolveResult[] resolveResults) {
        int maxRate = Integer.MIN_VALUE;
        for (ResolveResult resolveResult : resolveResults) {
            if (resolveResult instanceof RatedResolveResult ratedResolveResult) {
                int rate = ratedResolveResult.getRate();
                if (rate > maxRate) {
                    maxRate = rate;
                }
            }
     ...
     */
    @Nullable
    public static PyFunction getInitMethod(PyClass pyClass) {
     ...
     */
    @RequiredReadAction
    public static LanguageLevel getLanguageLevelForVirtualFile(Project project, VirtualFile virtualFile) {
        ...
        if (folder != null) {
            LanguageLevel folderLevel = folder.getUserData(LanguageLevel.KEY);
            if (folderLevel != null) {
                return folderLevel;
            }
            ...
            LanguageLevel level = virtualFile.getUserData(LanguageLevel.KEY);
            if (level != null) {
                return level;
            }
    ...
    public static void invalidateLanguageLevelCache(Project project) {
        project.putUserData(PythonLanguageLevelPusher.PYTHON_LANGUAGE_LEVEL, null);
    }

    @RequiredReadAction
    public static LanguageLevel guessLanguageLevelWithCaching(Project project) {
    ...
    @RequiredReadAction
    public static LanguageLevel guessLanguageLevel(Project project) {
    ...
    public static <T> T as(@Nullable Object expression, Class<T> clazz) {
    ...
    @Nullable
    @RequiredReadAction
    public static <T extends PsiElement> T findReference(PsiElement element, Class<T> expectedClass) {
        for (PsiReference reference : element.getReferences()) {
            T result = as(reference.resolve(), expectedClass);
            if (result != null) {
                return result;
            }
        }
     ...
     */
    public static <T> List<T> asList(@Nullable Collection<?> expression, Class<T> elementClass) {
        if ((expression == null) || expression.isEmpty()) {
            return Collections.emptyList();
        }
        List<T> result = new ArrayList<>();
        for (Object element : expression) {
            T toAdd = as(element, elementClass);
            if (toAdd != null) {
                result.add(toAdd);
            }
        }
     ...
     */
    @RequiredUIAccess
    public static void rehighlightOpenEditors(Project project) {
        project.getApplication().runWriteAction(() -> {
            for (Editor editor : EditorFactory.getInstance().getAllEditors()) {
                if (editor instanceof EditorEx editorEx && editorEx.getProject() == project) {
    ...
    public static <T, P> T getParameterizedCachedValue(PsiElement element, P param, Function<P, T> f) {
        Map<P, T> cache = LanguageCachedValueUtil.getCachedValue(
            element,
            new CachedValueProvider<Map<P, T>>() {
                @Nullable
                @Override
    ...
    public static void runWithProgress(
        ...
        final Consumer<ProgressIndicator> function
    ) {
        if (modal) {
     ...
     */
    public static void verboseOnly(Runnable runnable) {
     ...
     */
    @Nullable
    public static PsiComment getCommentOnHeaderLine(PyStatementListContainer container) {
     ...
     */
    @Nullable
    public static PsiElement getPackageElement(PsiDirectory directory, @Nullable PsiElement anchor) {
        if (isPackage(directory, anchor)) {
            PsiElement init = turnDirIntoInit(directory);
            if (init != null) {
                return init;
            }
            return directory;
    ...
    public static PsiElement turnInitIntoDir(@Nullable PsiElement target) {
        if (target instanceof PyFile file && isPackage(file)) {
     ...
     */
    public static boolean isPackage(PsiDirectory directory, @Nullable PsiElement anchor) {
     ...
     */
    @RequiredReadAction
    public static boolean isPackage(PsiDirectory directory, boolean checkSetupToolsPackages, @Nullable PsiElement anchor) {
        if (directory.findFile(PyNames.INIT_DOT_PY) != null) {
            return true;
        }
        LanguageLevel level = anchor != null
    ...
    @RequiredReadAction
    public static boolean isPackage(PsiFile file) {
        return PyNames.INIT_DOT_PY.equals(file.getName());
    }

    private static boolean isSetuptoolsNamespacePackage(PsiDirectory directory) {
    ...
    @Nullable
    private static String getPackagePath(PsiDirectory directory) {
        QualifiedName name = QualifiedNameFinder.findShortestImportableQName(directory);
     ...
     */
    public static LookupElement createNamedParameterLookup(String name, @Nullable Project project) {
        String suffix;
        if (CodeStyleSettingsManager.getSettings(project)
            .getCustomSettings(PyCodeStyleSettings.class).SPACE_AROUND_EQ_IN_KEYWORD_ARGUMENT) {
            suffix = " = ";
        }
        else {
            suffix = "=";
        }
        LookupElementBuilder lookupElementBuilder = LookupElementBuilder.create(name + suffix).withIcon(PlatformIconGroup.nodesParameter());
    ...
    public static PyExpression peelArgument(PyExpression expr) {
        while (expr instanceof PyParenthesizedExpression parenExpr) {
            expr = parenExpr.getContainedExpression();
        }
        if (expr instanceof PyKeywordArgument keywordArg) {
            expr = keywordArg.getValueExpression();
        }
        return expr;
    }
    ...
    public static String getFirstParameterName(PyFunction container) {
        ...
        if (container != null) {
            PyParameter[] params = container.getParameterList().getParameters();
            if (params.length > 0) {
                PyNamedParameter named = params[0].getAsNamed();
                if (named != null) {
     ...
     */
    @RequiredReadAction
    public static Collection<VirtualFile> getSourceRoots(PsiElement foothold) {
        Module module = foothold.getModule();
        if (module != null) {
     ...
     */
    public static Collection<VirtualFile> getSourceRoots(Module module) {
    ...
    public static List<String> strListValue(PyExpression value) {
        while (value instanceof PyParenthesizedExpression parenExpr) {
            value = parenExpr.getContainedExpression();
        }
        if (value instanceof PySequenceExpression sequenceExpr) {
            PyExpression[] elements = sequenceExpr.getElements();
    ...
    public static Map<String, PyExpression> dictValue(PyDictLiteralExpression dict) {
        ...
        for (PyKeyValueExpression keyValue : dict.getElements()) {
            ...
            if (key instanceof PyStringLiteralExpression stringLiteral) {
                result.put(stringLiteral.getStringValue(), value);
            }
        }
        return result;
    }
     ...
     */
    @SafeVarargs
    public static <T> boolean among(T what, T... variants) {
        for (T s : variants) {
            if (what.equals(s)) {
                return true;
            }
    ...
    public static class MethodFlags {
        ...
        @Nullable
        @RequiredReadAction
        public static MethodFlags of(PyFunction node) {
    ...
    @RequiredReadAction
    public static boolean isSuperCall(PyCallExpression node) {
        PyClass pyClass = PsiTreeUtil.getParentOfType(node, PyClass.class);
        if (pyClass == null) {
            return false;
        }
        ...
        if (PyNames.SUPER.equals(name)) {
            ...
            if (resolved != null && cache.isBuiltin(resolved)) {
                ...
                if (args.length > 0) {
                    ...
                    if (firstArg.equals(pyClass.getName()) || firstArg.equals(PyNames.CANONICAL_SELF + "." + PyNames.__CLASS__)) {
                        return true;
                    }
                    for (PyClass s : pyClass.getAncestorClasses(null)) {
    ...
    public static PyFile getOrCreateFile(String path, Project project) {
        ...
        if (vFile == null) {
            ...
            try {
                ...
                psi = PyExtractSuperclassHelper.placeFile(
                    project,
        ...
        if (psi instanceof PyFile pyFile) {
            return pyFile;
        }
        throw new IncorrectOperationException(PyLocalize.refactoringMoveModuleMembersErrorCannotPlaceElementsIntoNonpythonFile().get());
    }
    ...
    public static int getElementsCount(PyExpression expression, TypeEvalContext evalContext) {
        ...
        if (type instanceof PyTupleType tupleType) {
            valuesLength = tupleType.getElementCount();
        }
        else if (type instanceof PyNamedTupleType namedTupleType) {
            valuesLength = namedTupleType.getElementCount();
        }
        else if (expression instanceof PySequenceExpression sequenceExpr) {
            valuesLength = sequenceExpr.getElements().length;
        }
        else if (expression instanceof PyStringLiteralExpression stringLiteral) {
            valuesLength = stringLiteral.getStringValue().length();
        }
        else if (expression instanceof PyNumericLiteralExpression) {
            valuesLength = 1;
        }
        else if (expression instanceof PyCallExpression call) {
            if (call.isCalleeText("dict")) {
                ...
                if (arguments.length > 0 && arguments[0] instanceof PySequenceExpression sequenceExpr) {
                    valuesLength = sequenceExpr.getElements().length;
                }
            }
        }
     ...
     */
    @RequiredWriteAction
    public static <T extends PyElement> T addElementToStatementList(T element, PyStatementList statementList) {
    ...
    public static PsiElement addElementToStatementList(
        ...
        PsiElement prevElem = PyPsiUtils.getPrevNonWhitespaceSibling(statementList);
        ...
        if (prevElem != null && onSameLine(statementList, prevElem)) {
            PsiDocumentManager manager = PsiDocumentManager.getInstance(statementList.getProject());
            Document document = manager.getDocument(statementList.getContainingFile());
            if (document != null) {
                ...
                String text = statementList.getStatements().length == 0 ? indentation + PyNames.PASS : indentation;
        ...
        if (firstChild == statementList.getLastChild() && firstChild instanceof PyPassStatement) {
            ...
            if (toTheBeginning && statements.length > 0) {
                PyDocStringOwner docStringOwner = PsiTreeUtil.getParentOfType(statementList, PyDocStringOwner.class);
                PyStatement anchor = statements[0];
                if (docStringOwner != null && anchor instanceof PyExpressionStatement expressionStmt
                    && expressionStmt.getExpression() == docStringOwner.getDocStringExpression()) {
                    PyStatement next = PsiTreeUtil.getNextSiblingOfType(expressionStmt, PyStatement.class);
                    if (next == null) {
                        return statementList.addAfter(element, anchor);
                    }
                    anchor = next;
                }
                while (anchor instanceof PyExpressionStatement expressionStmt) {
                    if (expressionStmt.getExpression() instanceof PyCallExpression call) {
                        PyExpression callee = call.getCallee();
                        if (isSuperCall(call) || callee != null && PyNames.INIT.equals(callee.getName())) {
                            PyStatement next = PsiTreeUtil.getNextSiblingOfType(anchor, PyStatement.class);
                            if (next == null) {
                                return statementList.addAfter(element, anchor);
                            }
                            anchor = next;
    ...
    public static List<List<PyParameter>> getOverloadedParametersSet(PyCallable callable, TypeEvalContext context) {
        List<List<PyParameter>> parametersSet = getOverloadedParametersSet(context.getType(callable), context);
        return parametersSet != null
            ? parametersSet
            : Collections.singletonList(Arrays.asList(callable.getParameterList().getParameters()));
    }

    @Nullable
    private static List<PyParameter> getParametersOfCallableType(PyCallableType type, TypeEvalContext context) {
        List<PyCallableParameter> callableTypeParameters = type.getParameters(context);
    ...
    @Nullable
    private static List<List<PyParameter>> getOverloadedParametersSet(@Nullable PyType type, TypeEvalContext context) {
        if (type instanceof PyUnionType unionType) {
            type = unionType.excludeNull(context);
        }

        if (type instanceof PyCallableType callableType) {
            List<PyParameter> results = getParametersOfCallableType(callableType, context);
            if (results != null) {
                return Collections.singletonList(results);
            }
        }
        else if (type instanceof PyUnionType unionType) {
            List<List<PyParameter>> results = new ArrayList<>();
            Collection<PyType> members = unionType.getMembers();
            for (PyType member : members) {
                if (member instanceof PyCallableType callableType) {
                    List<PyParameter> parameters = getParametersOfCallableType(callableType, context);
                    if (parameters != null) {
    ...
    public static List<PyParameter> getParameters(PyCallable callable, TypeEvalContext context) {
        List<List<PyParameter>> parametersSet = getOverloadedParametersSet(callable, context);
        assert !parametersSet.isEmpty();
        return parametersSet.get(0);
    }

    public static boolean isSignatureCompatibleTo(
        PyCallable callable,
        PyCallable otherCallable,
        TypeEvalContext context
    ) {
        List<PyParameter> parameters = getParameters(callable, context);
        List<PyParameter> otherParameters = getParameters(otherCallable, context);
        int optionalCount = optionalParametersCount(parameters);
        int otherOptionalCount = optionalParametersCount(otherParameters);
        int requiredCount = requiredParametersCount(callable, parameters);
        int otherRequiredCount = requiredParametersCount(otherCallable, otherParameters);
    ...
    private static int optionalParametersCount(List<PyParameter> parameters) {
        int n = 0;
    ...
    private static int requiredParametersCount(PyCallable callable, List<PyParameter> parameters) {
        return parameters.size() - optionalParametersCount(parameters) - specialParametersCount(callable, parameters);
    }

    private static int specialParametersCount(PyCallable callable, List<PyParameter> parameters) {
        int n = 0;
        ...
        if (callable.asMethod() != null) {
            ...
            PyParameter first = parameters.get(0);
    ...
    private static boolean hasPositionalContainer(List<PyParameter> parameters) {
        for (PyParameter parameter : parameters) {
            if (parameter instanceof PyNamedParameter namedParam && namedParam.isPositionalContainer()) {
                return true;
            }
        }
        return false;
    ...
    private static boolean hasKeywordContainer(List<PyParameter> parameters) {
        for (PyParameter parameter : parameters) {
            if (parameter instanceof PyNamedParameter namedParam && namedParam.isKeywordContainer()) {
                return true;
            }
        }
        return false;
    ...
    @RequiredReadAction
    public static boolean isInit(PyFunction function) {
     ...
     */
    public static Collection<PyMemberInfo<PyElement>> filterOutObject(Collection<PyMemberInfo<PyElement>> pyMemberInfos) {
        return Collections2.filter(pyMemberInfos, new ObjectPredicate(false));
    }

    public static boolean isStarImportableFrom(String name, PyFile file) {
    ...
    public static class ObjectPredicate extends NotNullPredicate<PyMemberInfo<PyElement>> {
        ...
        @Override
        public boolean applyNotNull(PyMemberInfo<PyElement> input) {
            return myAllowObjects == isObject(input);
        }

        private static boolean isObject(PyMemberInfo<PyElement> classMemberInfo) {
            PyElement element = classMemberInfo.getMember();
     ...
     */
    public static boolean isSymbolInPackage(
        PyQualifiedNameOwner symbol,
        ...
        QualifiedName qualifiedName = QualifiedName.fromDottedString(qualifiedNameString);
     ...
     */
    public static boolean isObjectClass(PyClass cls) {
    ...
    @RequiredReadAction
    public static boolean isInScratchFile(PsiElement element) {
        return ScratchFileService.isInScratchRoot(PsiUtilCore.getVirtualFile(element));
    }

    @Nullable
    public static PyType getReturnTypeOfMember(
        PyType type,
        String memberName,
        ...
        if (resolveResults != null) {
            ...
            for (RatedResolveResult resolveResult : resolveResults) {
                PyType returnType = getReturnType(resolveResult.getElement(), context);
    ...
    @Nullable
    private static PyType getReturnType(@Nullable PsiElement element, TypeEvalContext context) {
    ...
    @Nullable
    private static PyType getReturnType(@Nullable PyType type, TypeEvalContext context) {
        if (type instanceof PyCallableType callableType) {
        ...
        if (type instanceof PyUnionType unionType) {
            ...
            for (PyType pyType : unionType.getMembers()) {
                PyType returnType = getReturnType(pyType, context);
    ...
    public static boolean isEmptyFunction(PyFunction function) {
    ...
    private static boolean isPassOrRaiseOrEmptyReturn(PyStatement stmt) {
        ...
        if (stmt instanceof PyReturnStatement returnStmt && returnStmt.getExpression() == null) {
            return true;
        }
        return false;
    }
    ...
    private static boolean isStringLiteral(PyStatement stmt) {
        if (stmt instanceof PyExpressionStatement expressionStmt) {
            PyExpression expr = expressionStmt.getExpression();
    ...
    public static final class StringNodeInfo {
        ...
        public StringNodeInfo(ASTNode node) {
        ...
        public StringNodeInfo(PsiElement element) {
        ...
        public static PsiNamedElement findName(Iterable<PsiNamedElement> it, String name) {
            ...
            for (PsiNamedElement elt : it) {
                if (elt != null) {
                    ...
                    if (elt instanceof PyQualifiedExpression qualifiedExpr && qualifiedExpr.isQualified()) {
                        continue;
                    }
